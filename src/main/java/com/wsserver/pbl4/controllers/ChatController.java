...
public class ChatController {
    ...
    public void processPrivateMessage(@Payload PrivateChatMessageDTO message) {
        PrivateChatMessage savedMessage = privateChatMessageService.saveMessage(message);

        System.out.println("Sending message to " + savedMessage.getReceiverId());
        // send notification to the queue /user/{uid}/queue/messages
        template.convertAndSendToUser(savedMessage.getReceiverId(), "/queue/messages",
                ChatNotification.builder()
                        .chatRoomId(savedMessage.getChatRoomId())
                        .senderId(savedMessage.getSenderId())
